...
$assert(str_contains($reviewFeatured, "'success' => true") && str_contains($reviewFeatured, "'reviews' =>") && str_contains($reviewFeatured, 'LIMIT 50') && str_contains($reviewFeatured, 'count($selected) >= 12'), 'featured review endpoint has a bounded successful public response and cap');
$assert(str_contains($reviewFeatured, 'ORDER BY vr.created_at DESC, vr.id DESC') && str_contains($reviewFeatured, "isset(\$selectedKeys[\$candidate['review_key']])") && str_contains($reviewFeatured, "isset(\$selectedIds[\$candidate['id']])") && str_contains($reviewFeatured, 'foreach ($candidates as $candidate)'), 'featured reviews preserve deterministic newest-first diversity before filling repeats');
$assert(str_contains($reviewFeatured, "moderation_status = 'Approved'") && str_contains($reviewFeatured, "TRIM(COALESCE(vr.review_text, '')) <> ''") && str_contains($reviewFeatured, "booking_status <> 'Cancelled'") && str_contains($reviewFeatured, "'Refunded'") && str_contains($reviewFeatured, "v.status = 'Available'") && str_contains($reviewFeatured, "v.category IN ('Event Hall', 'Hotel Room', 'Resort Villa')"), 'featured reviews enforce moderation, text, booking, availability, and category eligibility');
$assert(str_contains($reviewFeatured, "'rating' =>") && str_contains($reviewFeatured, "'review_text' =>") && str_contains($reviewFeatured, "'reviewer' =>") && str_contains($reviewFeatured, "'created_at' =>") && str_contains($reviewFeatured, "'category' =>") && str_contains($reviewFeatured, "'venue_name' =>") && str_contains($reviewFeatured, "'room_type' =>") && str_contains($reviewFeatured, "'review_key' =>") && !str_contains($reviewFeatured, "'customer_id' =>") && !str_contains($reviewFeatured, "'booking_id' =>") && !str_contains($reviewFeatured, "'email' =>") && !str_contains($reviewFeatured, "'phone' =>") && !str_contains($reviewFeatured, "'admin_note' =>"), 'featured response uses only the homepage privacy allowlist');
$assert(str_contains($reviewFeatured, 'strip_tags') && str_contains($reviewFeatured, 'mb_substr') && str_contains($reviewFeatured, "md5(\$venueNameRaw . ' - ' . \$roomTypeRaw)") && str_contains($reviewFeatured, "'event-' . (int)\$row['venue_id']") && str_contains($reviewFeatured, "'villa-' . (int)\$row['venue_id']"), 'featured reviews strip markup, mask names, and build stable event, villa, and hotel keys');
...
$assert(str_contains($dashboard, '<fieldset class="review-rating-fieldset"') && str_contains($dashboard, 'type="radio"') && str_contains($dashboard, 'name="review_rating"') && str_contains($dashboard, 'fa-solid fa-star review-star') && str_contains($dashboard, 'aria-hidden="true"') && str_contains($dashboard, 'out of 5'), 'customer rating uses accessible star radio controls');
$assert(str_contains($dashboardJs, 'review-rating-input') && str_contains($dashboardJs, 'input.checked') && str_contains($dashboardJs, 'reviewRating') && str_contains($dashboardJs, "classList.toggle('is-filled'") && str_contains($dashboardJs, 'value <= reviewRating') && str_contains($dashboardJs, 'reviewRating = 0;') && str_contains($dashboardJs, 'normalizeReviewRating') && str_contains($dashboardJs, "classList.add('is-submitting'") && str_contains($dashboardJs, "setAttribute('aria-busy', 'true')"), 'customer rating radios preserve exact selection and cumulatively paint, restore, and reset star state');
$assert(str_contains($dashboardCss, '.review-rating-fieldset') && str_contains($dashboardCss, '.review-rating-option.is-filled') && str_contains($dashboardCss, ':focus-visible') && str_contains($dashboardCss, '.btn-review') && str_contains($dashboardCss, '.review-modal-box #review-submit') && str_contains($dashboardCss, '#review-submit:hover:not(:disabled)') && str_contains($dashboardCss, '#review-submit:disabled') && str_contains($dashboardCss, '#review-submit.is-submitting') && str_contains($dashboardCss, '#review-submit[aria-busy="true"]') && str_contains($dashboardCss, '.review-modal-box #review-submit { background: var(--color-gold) !important; border: 1px solid var(--color-gold) !important; color: var(--color-dark) !important;') && str_contains($dashboardCss, '.review-modal-box #review-submit:hover:not(:disabled), .review-modal-box #review-submit:focus-visible { background: var(--color-gold-hover) !important; border-color: var(--color-gold-hover) !important; color: var(--color-dark) !important;'), 'customer rating controls retain compact focus and accessible scoped gold submit states');
...
$assert(!str_contains($index, 'top_rated_venues') && !str_contains($index, 'publicTopRatedVenues') && str_contains($index, "id=\"top-rated\"") && str_contains($index, 'hidden') && str_contains($index, 'idx-review-viewport') && str_contains($index, 'idx-review-track'), 'homepage review section starts hidden and no longer uses the top-rated threshold payload');
