...
$stmt_check = $conn->prepare("
    SELECT b.id, b.start_date, b.end_date, b.booking_status, v.category, vd.stay_type
    FROM bookings b 
    JOIN customers c ON b.customer_id = c.id 
    JOIN venues v ON b.venue_id = v.id
    LEFT JOIN booking_villa_details vd ON vd.booking_id = b.id
...
try {
    ...
    $req_start = DateTimeImmutable::createFromFormat('!Y-m-d', $new_start);
    if (!$req_start || $req_start->format('Y-m-d') !== $new_start || $req_start < new DateTimeImmutable('today')) throw new Exception('Please select a valid future reschedule date.');
    ...
    if ($booking['category'] === 'Resort Villa') {
        ...
        if ($booking['category'] === 'Hotel Room' && $nights < 1) {
            throw new Exception('The existing Hotel Room booking has an invalid duration.');
        }
        $req_end = $nights > 0 ? $req_start->modify("+{$nights} days") : $req_start;
    }
    $new_start = $req_start->format('Y-m-d');
    $new_end = $req_end->format('Y-m-d');
