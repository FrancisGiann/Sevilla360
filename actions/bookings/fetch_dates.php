...
if (isset($_GET['room_type']) && isset($_GET['room_name'])) {
    ...
    // This advanced query finds dates where ALL units of this specific room are completely booked!
    $stmt = $conn->prepare("
        ...
        HAVING COUNT(b.id) >= (
            SELECT COUNT(v2.id) 
            FROM venues v2 
            JOIN hotel_rooms h2 ON v2.id = h2.venue_id 
            WHERE h2.room_type = ? AND v2.name = ? AND v2.status = 'Available'
        )
    ");
